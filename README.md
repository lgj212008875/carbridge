...
> ⚠️ **项目定位说明**
> 本项目并非大而全的通用 OBD 诊断工具，而是针对**特定硬件环境（车机蓝牙被系统占用）**设计的一套**三端分离架构参考实现**。
> 
> **硬性限制：**
> 1. 仅支持 OBD-II KWP2000 协议（`AT SP 5`），不支持 CAN 总线车辆。
> 2. 仅测试过 ELM327 蓝牙版（推荐 v2.1 以上）。
> 3. 本项目的核心价值在于**架构设计**（蓝牙透传网关、云端解析下发、油耗加权校准），而非底层协议的广泛兼容性。
> 
> **架构通用性：**
> 虽然本项目针对"车机蓝牙被占用"场景做了三端分离，但 **BLE/SPP 透传 → 云端解析 → 纯展示 UI** 的架构同样适用于更简单的 **平板+OBD 直连方案**。只需将 ObdRelay 与 CarAgent 合并为单 App，运行在带 GPS 的 Android 平板上即可实现完整的车辆状态监控，无需 VPS。
>
> 如果你需要通用 OBD 工具，请使用 Torque 等。如果你是在学习如何设计复杂的 IoT 多端交互系统，你来对了地方。
...
- **农历时间显示**（纯查表算法，零依赖）
- **200ms 刷新率** — ARMv8 车机流畅运行（实测紫光展锐 7870）
